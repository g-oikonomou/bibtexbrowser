...
@define('BIBTEXBROWSER_EMBEDDED_WRAPPER', 'EmbeddedWrapper');
...
class BibEntry {
  ...
  */
  function toTR() {
        ...
        echo " [ <a".(BIBTEXBROWSER_BIB_IN_NEW_WINDOW?' target="_blank" ':'')." class=\"biburl\" title=\"".$this->getKey()."\" {$href}>full info</a> ]";

        echo ' - [ <a href="' . $this->getURL() . '&amp;astext" title="bib entry">bib</a> ]';

        $absId = 'abs'.$this->getKey();
        if ($this->hasField('abstract')) {
            echo ' - [ <a href="#'.$absId.'" title="Expand/Collapse" onclick="hideshow('."'".$absId."'".'); return false;">abstract</a> ]';
        }
        ...
        if ($this->hasField('abstract')) {
            echo '<div class="abstract" id="abs'.$this->getKey().'">'.$this->getAbstract().'</div>';
        }
...
class BibtexBrowserDisplay {
  ...
  function  formatedHeader() { return "<h1 class=\"rheader citation_title\">{$this->title}</h1>\n";}
  ...
  function display(&$display) {
    ...
    foreach ($years as $year => $entries) {
      ...
      if ($display->isDisplayed($index)) {
        ...
        echo "\n".'<div class="header">';
        echo $year.'</div>'."\n";
        echo '<table class="result" id="'.$sectionId.'">'."\n";
      }
...
class AcademicDisplay extends BibtexBrowserDisplay {
   ...
   */
  function search2html($query, $title, $sectionId) {
  ...
  function display() {
    foreach (_DefaultBibliographySections() as $sectionId => $section) {
      $this->search2html($section['query'],$section['title'], $sectionId);
    }
...
class BibEntryDisplay extends BibtexBrowserDisplay {
  ...
  function displayOnSteroids() {
      $subtitle = '<h3 class="bibentry-by citation_author">by '.$this->bib->getFormattedAuthorsImproved().'</h3>';
      ...
      $download = '';
      if ($this->bib->hasField('url') || $this->bib->hasField('doi')) {
        $download = '<div class="bibentry-label">Download Links:</div>';
        $download .= '<div class="bibentry-document-link">';
        if($this->bib->hasField('url')) {
          $download .= '[ <a href="'.$this->bib->getField('url').'">pdf</a> ]';
        }
        if($this->bib->hasField('doi')) {
          if($this->bib->hasField('url')) { $download .= ' - '; }
          $download .= '[ <a href="http://dx.doi.org/'.$this->bib->getField('doi').'">doi</a> ]';
        }
        $download .= '</div>';
      }

      $keypdf = dirname(__FILE__).'/Files/'.$this->bib->getKey().'.pdf';
      if (is_file($keypdf) && is_readable($keypdf)) {
        $disclaimer = getPublisherDisclaimer($this->bib);
        $preprint = '<div class="bibentry-label">Final Draft:</div>';
        if($disclaimer != '') {
          $preprint .= '<div class="bibentry-abstract">'.$disclaimer.'</div>';
        }
        $preprint .= '<div class="bibentry-document-link"><a href="Files/'.$this->bib->getKey().'.pdf">PDF</a></div>';
      }
      ...
      return $subtitle.$abstract.$download.$preprint.$reference.$bibtex.$this->bib->toCoins();
  }
  ...
  function metadata() {
    ...
    $keypdf = dirname(__FILE__).'/Files/'.$this->bib->getKey().'.pdf';
    if (is_file($keypdf) && is_readable($keypdf)) {
      $result[] = array('citation_pdf_url','http://' . $_SERVER['SERVER_NAME'] . '/Files/' . $this->bib->getKey().'.pdf');
    } else if ($this->bib->hasField("url")) {
...
/** NoWrapper calls method display() on the content.
  * To be used when we run with astext or to display an RSS feed */
class NoWrapper {
...
/** EmbeddedWrapper: to be used when we are included by another script
  * The includer can override by defining BIBTEXBROWSER_EMBEDDED_WRAPPER */
class EmbeddedWrapper {
  function EmbeddedWrapper(&$content, $metatags=array()) {
    @header('Content-type: text/html; charset='.ENCODING);
    echo $content->display();
  }
}
...
class Dispatcher {
  ...
  function Dispatcher() {
    ...
    if (strtr(realpath(__FILE__),"\\","/")!=strtr(realpath($_SERVER['SCRIPT_FILENAME']),"\\","/")) {
      ...
      $redir = "Location: pubs.php";
      if(strlen($_SERVER['QUERY_STRING']) > 0) { $redir .= '?' . $_SERVER['QUERY_STRING']; }
      header($redir);
      exit;
    }
  ...
  function key() {
    if ($_GET[Q_DB]->contains($_GET[Q_KEY])) {
      ...
      if(isset($_GET['astext'])) {
        ...
        /* BibtexDisplay expects an array of BibEntry, so we can not pass the
         * single BibEntry by itself */
        $this->selectedEntries[] = $bibdisplay->bib;

        $x = new $this->displayer($this->selectedEntries);
        new $this->wrapper($x);
      } else {
